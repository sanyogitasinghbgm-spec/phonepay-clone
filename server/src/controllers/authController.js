...
// Generates a unique UPI ID, retrying on the rare chance of a collision
const generateUniqueUpiId = async (name) => {
  ...
  while (exists) {
    ...
    upiId = `${sanitizedName}${randomSuffix}@phonepe`;
    exists = await User.findOne({ upiId });
  }
...
const registerUser = async (req, res) => {
  try {
    ...
    // Dynamic UPI Generation (Example: amit954123@phonepe)
    const upiId = await generateUniqueUpiId(name);
...
// @desc    Set or Change 4-6 digit numeric MPIN
...
const setupMpin = async (req, res) => {
  try {
    ...
    if (!mpin || !/^\d{4,6}$/.test(mpin.toString())) {
      return res.status(400).json({ message: 'Please provide a valid numeric MPIN (4 to 6 digits)' });
    }
